...
def train_diffro(config: ASRConfig):
    ...
    if not os.path.exists(sft_checkpoint):
        sft_checkpoint = "/mnt/nvme1/Chatterbox-Multilingual-TTS-Fine-Tuning/output_trainig_partial/best_checkpoint/model.safetensors"
    ...
    for epoch in range(config.num_epochs):
        ...
        for batch in tqdm(dataloader, disable=not accelerator.is_main_process):
            with accelerator.accumulate(model.t3):
                ...
                # D. Forward Judge
                judge_emb_matrix = reward_model.emb.weight[:-1] 
